...
- **本机工作区**：启动脚本会按宿主机 OS 自动把你的 Desktop（Win/Mac）或 `~/po-agent-user-workspace`（Linux）挂进容器的 `/workspace`，agent 就能操作你本机的项目文件（见第 4 节）。
...
> 也可以直接 `docker compose up --build -d`，但 compose 要求 `WORKSPACE_HOST_DIR` 已设置（见第 4 节），否则报错退出。脚本会自动设好，推荐用脚本。
...
3. **添加项目**：浏览 `/workspace` 选中你的项目目录添加（见第 4 节）。

这些状态写入数据卷（见第 6 节），重启容器后仍在。
...
- **子进程在容器内执行**：agent 通过 `NodeProcessRunner` 跑的 `npm`、`git` 等命令在容器环境里执行，容器只有 Node 22，没有宿主机的其它工具链。挂载的本机项目里若有需要其它工具链的命令，可能跑不起来。
...
- **只挂一个工作区**：脚本只挂 Desktop（Win/Mac）或 `~/po-agent-user-workspace`（Linux）。要访问其它盘/目录，手动设 `WORKSPACE_HOST_DIR` 或在 compose 里加额外挂载。

## 12. 故障排查

| 现象 | 排查 |
|---|---|
| `docker compose up` 报 `WORKSPACE_HOST_DIR ... is not set` | 用启动脚本启动；或手动 set/export 该变量指向你要挂的宿主机目录 |
...
| 端口 51732 被占用 | 停掉占用进程，或改 compose 的宿主机端口（见第 7 节） |
| 改代码后容器没更新 | 生产模式要 `--build`（脚本已带）；开发模式确认用的是 `dev` 参数 |
| 凭证丢失 | 确认没有用 `down -v`；检查 `po-agent-data` 卷是否存在（`docker volume ls`） |
| UI 里浏览不到 `/workspace` 或看不到本机文件 | 确认用脚本启动（设了 `WORKSPACE_HOST_DIR`）；Docker Desktop 文件共享是否包含该路径（Win/Mac 默认共享用户目录） |
| 挂载路径有权限错误 | Linux 上确认 `~/po-agent-user-workspace` 属于当前用户；Win/Mac 用户目录默认可读写 |

## 13. 验证清单

- [ ] `.\scripts\docker-up.ps1`（或 `./scripts/docker-up.sh`）启动，`http://localhost:51732` 可访问
- [ ] UI 里"添加项目"能浏览 `/workspace`，看到本机 Desktop（或 Linux 工作区）的内容
- [ ] 添加 `/workspace/<某项目>` 后，agent 能读写该项目文件
- [ ] 登录一个 provider、配置一个模型
- [ ] 重启容器（经脚本）后，已添加项目和凭证仍在（数据卷生效）
- [ ] （可选）`scripts/docker-up.* dev` 启动，改源码后页面热更新
